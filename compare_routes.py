...
async def compare_tests(location: LocationInput):
    """
    Compare lab tests for a given location and list of competitors.
    """
    ...
    try:
        ...
        if "Incorrect API key provided" in str(e) or "OpenAI Embedding API failed" in str(e):
            raise HTTPException(status_code=500, detail="OpenAI API key error: Please check your API key.")
...
async def analyze_endpoint(payload: AnalyzeRequest):
    """
    Analyze prices and recommend labs per test.
    """
