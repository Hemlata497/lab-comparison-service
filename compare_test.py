    ...
    for i, (source_name, embed) in enumerate(zip(source_names, source_embeds)):
        ...
        for idx in sorted_indices:
            if sims[idx] >= threshold and target_names[idx] not in used_targets:
                ...
                break
        if not found:
            print(f"No match for: {source_name} (max similarity: {max(sims):.3f})")
    return matches
...
async def run_scrapers_and_compare(location: str):
    ...
    print("Running scrapers in parallel: LalPathLabs | SRL | Metropolis...")
    ...
    async def run_scraper_py(script, input_str, output_file):
        ...
        stdout, stderr = await proc.communicate(input=input_str.encode())
        if stdout:
            print(f"[{script} output]", stdout.decode())
        if stderr:
            print(f"[{script} error]", stderr.decode())
        # Wait for output file to appear (max 15s)
        for _ in range(30):
    ...
    await asyncio.gather(
        run_scraper_py("lalpathlabs.py", f"{location}\n", lal_file),
        run_scraper_py("srl.py", "\n", srl_file),
        run_scraper_py("metropolisindia.py", f"{location}\n", metro_file)
    )
    ...
    if os.path.exists(lal_file):
        async with aiofiles.open(lal_file, mode='r', encoding='utf-8') as f:
            ...
            lal_tests = [(t["test_name"].strip(), t["price"].strip()) for t in json.loads(lal_data) if t["test_name"] != "N/A"]
    else:
    ...
    if os.path.exists(srl_file):
        async with aiofiles.open(srl_file, mode='r', encoding='utf-8') as f:
            ...
            srl_tests = [(t["test"].strip(), t["price"].replace("₹", "").strip()) for t in json.loads(srl_data) if t["test"] != "N/A"]
    else:
    ...
    if not (lal_tests and metro_tests and srl_tests):
        print("❌ Not enough data from all sources to perform comparison. Check if all scrapers produced valid output.")
        return
